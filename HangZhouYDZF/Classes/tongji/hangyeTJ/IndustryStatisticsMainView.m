...
@implementation IndustryStatisticsMainView
...
- (void)popingInputView {
	...
	popView.industryParentView = self;
    popView.parentTag = 1;
...
- (void)initWithBeginDate:(NSString *)bdValue endDate:(NSString *)edValue {
    ...
    NSString *param = [WebServiceHelper createParametersWithKey:@"beginDate" value:bdValue,@"endDate",edValue,@"orgid",g_logedUserInfo.userORGID,@"userName",g_logedUserInfo.userName,nil];
    
     NSString *URL = [NSString  stringWithFormat: kTaskService_URL,g_appDelegate.ServerIP];
    webservice = [[[WebServiceHelper alloc] initWithUrl:URL method:@"GetTaskBasicsByIndustryType_Log" view:self.view nameSpace:@"http://Services.MobileLaw.Powerdata.com/" parameters:param delegate:self] autorelease];
    webservice.hudTitle = @"正在查询行业统计结果,请稍候...";
    [webservice run];
}
...
- (void)viewDidLoad
{
    ...
    self.beginDateValue = [[[NSString alloc] initWithFormat:@"%@",[[fromday description] substringToIndex:10]] autorelease];
    ...
    NSString *URL = [NSString  stringWithFormat: kTaskService_URL,g_appDelegate.ServerIP];
    webservice = [[[WebServiceHelper alloc] initWithUrl:URL method:@"GetTaskBasicsByIndustryType_Log_UDID" view:self.view nameSpace:@"http://Services.MobileLaw.Powerdata.com/" parameters:param delegate:self] autorelease];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![[staCountArr objectAtIndex:row] isEqualToString:@"0"]&&![[staCodeArr objectAtIndex:row] isEqualToString:@""]) {
        [childView setBeginDateStr:self.beginDateValue];
        [childView setEndDateStr:self.endDateValue];
        [childView setIndustryStr:[staTypeArr objectAtIndex:row]];
