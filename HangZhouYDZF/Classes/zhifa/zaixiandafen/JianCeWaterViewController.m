...
@interface JianCeWaterViewController ()
...
@implementation JianCeWaterViewController
...
- (void)loadLocalSaveRecord{
    NSArray *chooseAry = [[DBHelper sharedInstance] queryRecordByWrymc:wrymc andTableName:tableName TYPE:@"water"];
...
-(void)requestServerHisRecord{
    NSString* params = [WebServiceHelper createParametersWithKey:@"tableName"														   value:tableName,@"xczfbh",xczfbh,@"lx",@"2",@"userName",g_logedUserInfo.userName,@"UDID",g_appDelegate.udid,nil];
...
- (void)refreshScore:(NSString *)currentScore{
    ...
    //detailViewController.tableIndex = tableIndex;
...
//    JCDW.text = [valueDic objectForKey:@"ZFJCDW"];
//    QYMC.text = [valueDic objectForKey:@"DWMC"];
//    QYDZ.text = [valueDic objectForKey:@"DWDZ"];
//    YWGS.text = [valueDic objectForKey:@"YWGS"];
...
-(void)saveLocalRecord:(NSDictionary *)valueDatas{
    ...
    BOOL res = [helper saveRecord:jsonStr XCZFBH:xczfbh WRYMC:wrymc TableName:tableName JCR:g_logedUserInfo.userName TYPE:@"water"];
    [jsonStr release];
    ...
    [mainTable setObject:@"2" forKey:@"LX"];
    ...
    BOOL main = [helper saveRecord:jsonStr XCZFBH:xczfbh WRYMC:wrymc TableName:@"T_YDZF_ZXXCDFB" JCR:g_logedUserInfo.userName TYPE:@"water"];
    [jsonStr release];
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        total = 0;
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"watercheck" ofType:@"plist"];
...
{
    [super viewDidLoad];
    self.title = @"固定污染源废水自动监测系统现场核查表";
    scrollView.contentSize = CGSizeMake(768, 1300);
    ...
    masterTableView.contentSize = CGSizeMake(280, 1140);
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if (self.isHisRecord) { NSArray *subViews = [scrollView subviews];
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    if (currentData &&[ elementName isEqualToString:@"GetChildsDetialsjson_Log_UDIDResult"]) {
        ...
        for (int index = 0; index < [dataZB count]; index++) {
            ...
            else if([contentMC isEqualToString:@"PWKGFHPFNR1"]) {
                [zbAry replaceObjectAtIndex:1 withObject:item];
            }
            else if([contentMC isEqualToString:@"PWKGFHPFNR2"]) {
                [zbAry replaceObjectAtIndex:2 withObject:item];
            }
            else if([contentMC isEqualToString:@"PWKGFHPFNR3"]) {
                [zbAry replaceObjectAtIndex:3 withObject:item];
            }
            else if([contentMC isEqualToString:@"YQAZPFNR1"]) {
                [zbAry replaceObjectAtIndex:4 withObject:item];
            }
            else if([contentMC isEqualToString:@"YQAZPFNR2"]) {
                [zbAry replaceObjectAtIndex:5 withObject:item];
            }
            else if([contentMC isEqualToString:@"YQAZPFNR3"]) {
            ...
            else if([contentMC isEqualToString:@"WRYJCXTPFNR2"]) {
                [zbAry replaceObjectAtIndex:8 withObject:item];
            }
            else if([contentMC isEqualToString:@"FZXTPFNR1"]) {
                [zbAry replaceObjectAtIndex:9 withObject:item];
            }
            else if([contentMC isEqualToString:@"FZXTPFNR2"]) {
                [zbAry replaceObjectAtIndex:10 withObject:item];
            }
            else if([contentMC isEqualToString:@"FZXTPFNR3"]) {
            ...
            else if([contentMC isEqualToString:@"FXYQJCPFNR3"]) {
                [zbAry replaceObjectAtIndex:14 withObject:item];
            }
            else if([contentMC isEqualToString:@"FXYQJCPFNR4"]) {
                [zbAry replaceObjectAtIndex:15 withObject:item];
            }
            else if([contentMC isEqualToString:@"BZRYJCPFNR1"]) {
                [zbAry replaceObjectAtIndex:16 withObject:item];
            }
            else if([contentMC isEqualToString:@"BZRYJCPFNR2"]) {
                [zbAry replaceObjectAtIndex:17 withObject:item];
            }
            else if([contentMC isEqualToString:@"FSXZXTPFNR1"]) {
                [zbAry replaceObjectAtIndex:18 withObject:item];
            }
            else if([contentMC isEqualToString:@"FSXZXTPFNR2"]) {
            ...
            else if([contentMC isEqualToString:@"FSSJYZXPFNR1"]) {
                [zbAry replaceObjectAtIndex:22 withObject:item];
            }
            else if([contentMC isEqualToString:@"FSSJYZXPFNR2"]) {
            ...
            else if([contentMC isEqualToString:@"SBCLPFNR1"]) {
            ...
            else if([contentMC isEqualToString:@"YXJLDZDAPFNR4"]) {
        ...
        for (int index = 0; index< [zbAry count]; index++) {
            ...
            NSString *FSXZXTBY      = [item objectForKey:@"FSXZXTBY"];
            NSString *FSXZXTZB      = [item objectForKey:@"FSXZXTZB"];
            NSString *FSXZXTSZ      = [item objectForKey:@"FSXZXTSZ"];
            NSString *FSSJYZXSC     = [item objectForKey:@"FSSJYZXSC"];
            NSString *FSSJYZXSZ     = [item objectForKey:@"FSSJYZXSZ"];
            NSString *FSSJYZXZB     = [item objectForKey:@"FSSJYZXZB"];
            ...
            [cellInfo setObject:FSXZXTBY  forKey:@"FSXZXTBY"];
            [cellInfo setObject:FSXZXTZB  forKey:@"FSXZXTZB"];
            [cellInfo setObject:FSXZXTSZ  forKey:@"FSXZXTSZ"];
            [cellInfo setObject:FSSJYZXSC forKey:@"FSSJYZXSC"];
            [cellInfo setObject:FSSJYZXSZ forKey:@"FSSJYZXSZ"];
            [cellInfo setObject:FSSJYZXZB forKey:@"FSSJYZXZB"];
            ...
            if ([contentAry count] > 1) {
                ...
                for (int j=index+1; j<[zbAry count]; j++) {
                    ...
                    NSString *FSXZXTBY0   = [itemTmp objectForKey:@"FSXZXTBY"];
                    NSString *FSXZXTZB0   = [itemTmp objectForKey:@"FSXZXTZB"];
                    NSString *FSXZXTSZ0   = [itemTmp objectForKey:@"FSXZXTSZ"];
                    NSString *FSSJYZXSC0  = [itemTmp objectForKey:@"FSSJYZXSC"];
                    NSString *FSSJYZXSZ0  = [itemTmp objectForKey:@"FSSJYZXSZ"];
                    NSString *FSSJYZXZB0  = [itemTmp objectForKey:@"FSSJYZXZB"];
                    ...
                    [cellInfo setObject:FSXZXTBY0  forKey:@"FSXZXTBY"];
                    [cellInfo setObject:FSXZXTZB0  forKey:@"FSXZXTZB"];
                    [cellInfo setObject:FSXZXTSZ0  forKey:@"FSXZXTSZ"];
                    [cellInfo setObject:FSSJYZXSC0 forKey:@"FSSJYZXSC"];
                    [cellInfo setObject:FSSJYZXSZ0 forKey:@"FSSJYZXSZ"];
                    [cellInfo setObject:FSSJYZXZB0 forKey:@"FSSJYZXZB"];
